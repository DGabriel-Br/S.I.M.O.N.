...
- primeiro consumidor de `INTERRUPT` com `interrupt-open`: um assessment urgente ou de risco pode virar `attention.interrupt.requested` persistente; enquanto o pedido estiver `PENDING`, o Executive retorna `NEEDS_INTERRUPT_REVIEW` antes de iniciar qualquer nova transição, sem pausar Goal ou Plan, interromper Action, trocar foco ou avançar `world_revision`.
- lifecycle humano de itens `ATTEND` com `attention-review`: itens `PENDING` podem terminar como `DISMISSED`, `ACKNOWLEDGED` ou `GOAL_PROPOSED`; a terceira opção materializa uma `attention.goal_proposal.completed` estruturada pelo usuário e ainda exige `goal-accept` separado, enquanto nenhuma review altera foco ou `world_revision`.
- revisão conversacional determinística de `ATTEND` via `user-turn`: ordinais ou item único podem ser dispensados ou reconhecidos por linguagem curta, e uma transformação em Goal aceita campos explícitos `título`, `estado` e `critério`; a review reutiliza o lifecycle do Passo 81 e propostas originadas dessa conversa podem ser aceitas ou rejeitadas no turno seguinte com `sim`/`não`.
